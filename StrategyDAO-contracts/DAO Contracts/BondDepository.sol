...
contract StrategyDAOBondDepository is Ownable {
    ...
    address public immutable STRAT; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints STRAT when receives principle
    ...
    uint32 public lastDecay; // reference STRAT for debt decay
    ...
    struct Bond {
        uint payout; // STRAT remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 STRAT ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) STRAT
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( STRAT ).safeTransfer( DAO, fee ); 
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( STRAT ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( STRAT ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( STRAT ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( STRAT ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to STRAT supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( STRAT ).totalSupply();
    ...
    /**
     *  @notice calculate amount of STRAT available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or STRAT) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != STRAT );
