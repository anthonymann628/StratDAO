...
contract StrategyDAOTreasury is Ownable {
    ...
    address public immutable Strat;
    ...
    address public sSTRAT;
    uint public sSTRATQueue; // Delays change to sSTRAT address
    ...
    //    isLiquidityToken[ _STRATDAI ] = true;
    //    liquidityTokens.push( _STRATDAI );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for STRAT
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint STRAT needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Strat ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn STRAT for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ISTRATERC20( Strat ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sSTRAT ).balanceOf( msg.sender ); // Can only borrow against sSTRAT held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with STRAT
        @param _amount uint
     */
    function repayDebtWithSTRAT( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISTRATERC20( Strat ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Strat, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Strat ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Strat ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns STRAT valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match STRAT decimals
            value_ = _amount.mul( 10 ** IERC20( Strat ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SSTRAT ) { // 9
            sSTRATQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
